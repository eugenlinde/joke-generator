...
describe('SavedJokes Component', () => {
    ...
    it('renders a message when there are no saved jokes', () => {
        ...
        const { getByText } = render(<Router ><SavedJokes /></Router>);
        expect(getByText("No saved jokes? Chuck Norris once made a joke so funny, it saved itself. Looks like you're not quite there yet.")).toBeInTheDocument();
    });

    it('renders saved jokes grouped by category', () => {
        const savedJokes: Joke[] = [
            { value: 'Joke 1', category: 'Category 1', date: new Date().toISOString() },
            { value: 'Joke 2', category: 'Category 2', date: new Date().toISOString() },
            { value: 'Joke 3', category: 'Category 1', date: new Date().toISOString() },
        ];
